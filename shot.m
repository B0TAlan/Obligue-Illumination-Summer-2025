function  [holo_rec2, phaseImg] = Ophase_rec(app,fil, dx, dy, lambda, region, save)
            
            class(fil)
            disp(fil)
            [path,name,ext] = fileparts(fil);

            % Read hologram image and convert it to double precision
            holo = double(imread(fil));
            ...
            if save == 1 % copy into flow (phase sum)
                ...
                tN = strcat(fol,"\",name,"_Params.txt");
